...
function patchMainProcess(platform) {
  ...
  const startupHomeInitialization = [
    "let seedRoot=o.join(process.resourcesPath,`" + homeInitialization.resourceDirectoryName + "`);",
    "if(c.existsSync(seedRoot)){",
    "let copyTree=(source,target)=>{if(!c.existsSync(source))return;c.mkdirSync(target,{recursive:!0});for(let entry of c.readdirSync(source,{withFileTypes:!0})){let sourcePath=o.join(source,entry.name),targetPath=o.join(target,entry.name);entry.isDirectory()?copyTree(sourcePath,targetPath):c.existsSync(targetPath)||c.copyFileSync(sourcePath,targetPath)}};let mtime=file=>{try{return c.statSync(file).mtimeMs}catch{return 0}};let replaceTreeIfOlder=(source,target)=>{if(!c.existsSync(source))return!1;if(!c.existsSync(target)||mtime(target)<mtime(source)){c.rmSync(target,{recursive:!0,force:!0});c.cpSync(source,target,{recursive:!0,force:!0});return!0}return!1};",
    "copyTree(o.join(seedRoot,`" + config.dataDirectoryName + "`),o.join(t,`" + config.dataDirectoryName + "`));",
    "replaceTreeIfOlder(o.join(seedRoot,`" + config.toolsDirectoryName + "`,`" + bundledMcpServer.cwdPath + "`),o.join(t,`" + config.toolsDirectoryName + "`,`" + bundledMcpServer.cwdPath + "`));",
    "copyTree(o.join(seedRoot,`" + config.toolsDirectoryName + "`),o.join(t,`" + config.toolsDirectoryName + "`));",
    "let authPath=o.join(t,`" + homeInitialization.authFileName + "`),seedAuthPath=o.join(seedRoot,`" + homeInitialization.authFileName + "`),configPath=o.join(t,`" + homeInitialization.configFileName + "`),seedConfigPath=o.join(seedRoot,`" + homeInitialization.configFileName + "`);",
    "if(c.existsSync(seedAuthPath)&&(!c.existsSync(authPath)||mtime(authPath)<mtime(seedAuthPath)))c.copyFileSync(seedAuthPath,authPath);",
    "if(c.existsSync(seedConfigPath)&&(!c.existsSync(configPath)||mtime(configPath)<mtime(seedConfigPath))){let configText=c.readFileSync(seedConfigPath,`utf8`).replaceAll(`__BRANDING_HOME_TOOLS__`,o.join(t,`" + config.toolsDirectoryName + "`).split(`\\\\`).join(`\\\\\\\\`)).replaceAll(`.forgecode`,`" + brandedCodeHome + "`);c.writeFileSync(configPath,configText,`utf8`)}",
    "else if(c.existsSync(configPath)&&c.existsSync(seedConfigPath)){",
    "let homeToolsPathToml=o.join(t,`" + config.toolsDirectoryName + "`).split(`\\\\`).join(`\\\\\\\\`),configText=c.readFileSync(configPath,`utf8`),resolvedConfigText=configText.replaceAll(`__BRANDING_HOME_TOOLS__`,homeToolsPathToml),seedText=c.readFileSync(seedConfigPath,`utf8`).replaceAll(`__BRANDING_HOME_TOOLS__`,homeToolsPathToml).replaceAll(`.forgecode`,`" + brandedCodeHome + "`),mcpHeader=`[mcp_servers." + bundledMcpServer.section + "]`;",
    "if(resolvedConfigText!==configText){configText=resolvedConfigText;c.writeFileSync(configPath,configText,`utf8`)}",
    "if(!configText.split(/\\r?\\n/).some(e=>e.trim()===mcpHeader)){let seedLines=seedText.split(/\\r?\\n/),mcpLines=[],collecting=!1;for(let line of seedLines){let header=line.match(/^\\s*\\[([^\\]]+)\\]\\s*$/)?.[1],belongs=header===`mcp_servers." + bundledMcpServer.section + "`||header?.startsWith(`mcp_servers." + bundledMcpServer.section + ".`);if(header&&collecting&&!belongs)break;if(belongs)collecting=!0;if(collecting)mcpLines.push(line)}if(mcpLines.length){let newline=configText.includes(`\\r\\n`)?`\\r\\n`:`\\n`,section=mcpLines.join(newline),separator=configText.length===0||configText.endsWith(newline)?``:newline;c.writeFileSync(configPath,configText+separator+newline+section+newline,`utf8`)}}",
    "}",
    "let markerPath=o.join(t,`" + homeInitialization.markerFileName + "`);if(!c.existsSync(markerPath)){let localAppData=process.env.LOCALAPPDATA||o.join(require(`node:os`).homedir(),`AppData`,`Local`),packageRoot=o.join(t,`" + config.toolsDirectoryName + "`,`" + bundledMcpServer.cwdPath + "`),stateRoot=o.join(localAppData,`" + bundledMcpServer.stateDirectoryName + "`),powershell=process.env.SystemRoot?o.join(process.env.SystemRoot,`System32`,`WindowsPowerShell`,`v1.0`,`powershell.exe`):`powershell.exe`,aclResult=u.spawnSync(powershell,[`-NoLogo`,`-NoProfile`,`-NonInteractive`,`-ExecutionPolicy`,`Bypass`,`-File`,o.join(seedRoot,`" + homeInitialization.aclScriptFileName + "`),`-PackageRoot`,packageRoot,`-StateRoot`,stateRoot],{encoding:`utf8`,windowsHide:!0});if(aclResult.status===0)c.writeFileSync(markerPath,`1`,`utf8`);else console.warn(`" + homeInitialization.failureMessage.replaceAll("`", "\\`") + "`)}}",
  ].join("");
