...
function patchMainProcess(platform) {
  ...
  const startupHomeInitialization = [
    ...
    "let authPath=o.join(t,`" + homeInitialization.authFileName + "`),legacyAuthPath=o.join(require(`node:os`).homedir(),`.forgecode`,`" + homeInitialization.authFileName + "`),seedAuthPath=o.join(seedRoot,`" + homeInitialization.authFileName + "`),configPath=o.join(t,`" + homeInitialization.configFileName + "`),seedConfigPath=o.join(seedRoot,`" + homeInitialization.configFileName + "`);",
    "if(c.existsSync(seedAuthPath)){let shouldCopyAuth=!c.existsSync(authPath);if(!shouldCopyAuth&&c.existsSync(legacyAuthPath)){let currentAuth=c.readFileSync(authPath),legacyAuth=c.readFileSync(legacyAuthPath),seedAuth=c.readFileSync(seedAuthPath);shouldCopyAuth=currentAuth.equals(legacyAuth)&&!currentAuth.equals(seedAuth)}if(shouldCopyAuth)c.copyFileSync(seedAuthPath,authPath)}",
    "if(!c.existsSync(configPath)&&c.existsSync(seedConfigPath)){let configText=c.readFileSync(seedConfigPath,`utf8`).replaceAll(`__BRANDING_HOME_TOOLS__`,o.join(t,`" + config.toolsDirectoryName + "`).split(`\\\\`).join(`\\\\\\\\`)).replaceAll(`.forgecode`,`" + brandedCodeHome + "`);c.writeFileSync(configPath,configText,`utf8`)}",
